...
int main()
{
	...
	function<void(int)>dfs = [&](int u)//仿函数
	{
		for (auto v : g[u])
			dfs(v);
	};
...
void add(int u, int v) { to[tot] = v; nxt[tot] = h[u]; h[u] = tot++; }
int n, m;
void dfs(int u)
{
...
```c++
int gcd(int a, int b){return b == 0 ? a : gcd(b, a % b);}//求最小公倍数：a*b/gcd(a,b)
